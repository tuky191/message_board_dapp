...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::SubmitMessage {subject, content} => try_submit(deps, info, subject, content),
        ExecuteMsg::LikeMessage {index} => try_like(deps, info, index),
    }
}

pub fn try_submit(deps: DepsMut, info: MessageInfo, subject: String, content: String) -> Result<Response, ContractError> {
    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
        state.messages.push(Post::new(info.sender, subject, content));
        Ok(state)
    })?;
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetMessageByAddress { addr } => to_binary(&get_posts_by_addr(deps, addr)?)
    }
}
...
fn get_posts_by_addr(deps: Deps, addr: Addr) -> StdResult<MessagesResponse> {
    let state = STATE.load(deps.storage)?;
...
mod tests {
    ...
    fn proper_initialization() {
        ...
        let msg = InstantiateMsg { messages: Vec::new() };
   ...
   #[test]
    fn create_post() {
        ...
        // WHEN
        let msg = ExecuteMsg::SubmitMessage { content: String::from("some content"), subject : String::from("some subject")};
        println!("{:#?}", msg);
    ...
    fn like_a_created_post() {
        ...
        let _res = instantiate(deps.as_mut(), mock_env(), info, msg).unwrap();
        let info = mock_info("anyone", &coins(2, "token"));
        let msg = ExecuteMsg::SubmitMessage { subject : String::from("some subject"), content: String::from("some content")};
        let _res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
